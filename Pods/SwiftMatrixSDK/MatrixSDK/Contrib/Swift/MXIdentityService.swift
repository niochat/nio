...
public extension MXIdentityService {
    ...
    /**
     ...
     - parameters:
     - email: phoneNumber the phone number (in international or national format).
     - countryCode: countryCode the ISO 3166-1 country code representation (required when the phone number is in national format).
     - clientSecret: a secret key generated by the client. (`MXTools.generateSecret()` creates such key)
     - sendAttempt: the number of the attempt for the validation request. Increment this value to make the identity server resend the email. Keep it to retry the request in case the previous request failed.
     - nextLink: the link the validation page will automatically open. Can be nil
     - completion: A block object called when the operation completes. It provides The session IDsession ID and MSISDN.
     - response: Provides provides the id of the email validation session on success.
     
     - returns: a `MXHTTPOperation` instance.
     */
    @nonobjc @discardableResult func requestPhoneNumberValidation(_ phoneNumber: String, countryCode: String, clientSecret: String, sendAttempt: UInt, nextLink: String?, completion: @escaping (_ response: MXResponse<(String, String)>) -> Void) -> MXHTTPOperation {
        return __requestPhoneNumberValidation(phoneNumber, countryCode: countryCode, clientSecret: clientSecret, sendAttempt: sendAttempt, nextLink: nextLink, success: { (sid, msisdn) -> Void in
            let validationInformation = (sid, msisdn)
            completion(MXResponse.success(validationInformation))
